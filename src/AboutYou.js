...
export default function AboutYou({ onNext, onBack }) {
  ...
  return (
    <div style={{ width:'100%', minHeight:'100dvh', background:'#000', display:'flex', flexDirection:'column', position:'relative', overflow:'hidden', paddingLeft:'24px', paddingRight:'24px', paddingTop:'max(48px, env(safe-area-inset-top))', paddingBottom:'max(20px, env(safe-area-inset-bottom))', fontFamily:"'Pretendard',-apple-system,sans-serif" }}>
      ...
      <div className="glow-anim" style={{ position:'absolute', bottom:'-80px', right:'10%', width:'300px', height:'300px', borderRadius:'50%', background:'radial-gradient(circle,#6ee7b7 0%,transparent 70%)', filter:'blur(80px)', opacity:0.4, pointerEvents:'none', animationDelay:'0.5s' }} />
      <div className="glow-anim" style={{ position:'absolute', bottom:'-60px', left:'5%', width:'280px', height:'280px', borderRadius:'50%', background:'radial-gradient(circle,#f9a8d4 0%,transparent 70%)', filter:'blur(80px)', opacity:0.3, pointerEvents:'none', animationDelay:'1.5s' }} />
      ...
      {mounted && (
        <div style={{ marginTop:'24px', flexShrink:0 }}>
          <p className="display fade-up" style={{ fontSize:'11px', letterSpacing:'0.3em', fontWeight:900, textTransform:'uppercase', margin:'0 0 12px', color:'transparent', background:GRADIENT, WebkitBackgroundClip:'text', backgroundClip:'text', animationDelay:'0.1s' }}>Step 2 of 5</p>
          ...
          <p className="fade-up" style={{ fontSize:'12px', fontWeight:700, margin:0, color:TEXT_MUTED, animationDelay:'0.2s' }}>너에 대해 알고 싶어.</p>
        </div>
      )}
      ...
      {mounted && (
        <div className="fade-up" style={{ marginTop:'20px', flex:1, animationDelay:'0.35s' }}>
            ...
            {goals.map(g => {
              ...
              return (
                <button key={g.id} onClick={() => setGoal(g.id)}
                  ...
                  <div style={{ flex:1 }}>
                    <p className="display" style={{ fontSize:'16px', fontWeight:900, color: isSelected ? '#a78bfa' : TEXT, margin:0 }}>{g.label}</p>
