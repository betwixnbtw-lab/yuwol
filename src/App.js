...
function Welcome({ onNext }) {
  return (
    <>
      ...
      <div style={{ width:'100%', height:'100dvh', background:'#000', display:'flex', flexDirection:'column', position:'relative', overflow:'hidden', paddingLeft:'28px', paddingRight:'28px', paddingTop:'max(48px, env(safe-area-inset-top))', paddingBottom:'max(32px, env(safe-area-inset-bottom))' }}>
...
export default function App() {
  ...
  if (screen === 'invite') {
    return <InviteCode onNext={() => setScreen('welcome')} onBack={() => setScreen('welcome')} />;
  }
