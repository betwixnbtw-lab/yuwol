...
export default function InviteCode({ onNext, onBack }) {
  ...
  return (
    <div
      ...
      style={{ fontFamily: "'Pretendard', -apple-system, sans-serif", background: '#000' }}
    >
      ...
      <div
        ...
        style={{ aspectRatio: '9/19', minHeight: '720px', background: '#000', border: '6px solid #1a1a1a' }}
      >
        <div className="absolute -top-32 -right-20 w-80 h-80 rounded-full pointer-events-none glow-anim"
          style={{ background: 'radial-gradient(circle, #a78bfa 0%, transparent 70%)', filter: 'blur(70px)' }} />
        <div className="absolute top-1/3 -left-32 w-72 h-72 rounded-full pointer-events-none glow-anim"
          style={{ background: 'radial-gradient(circle, #6ee7b7 0%, transparent 70%)', filter: 'blur(70px)', animationDelay: '1s' }} />
        ...
        <div className="relative flex flex-col h-[calc(100%-32px)] px-6 pb-5">
          ...
          {mounted && (
            <div className="flex items-center justify-between mt-4 fade-up" style={{ animationDelay: '0.05s' }}>
              <button className="w-10 h-10 rounded-full flex items-center justify-center"
                style={{ background: 'rgba(255,255,255,0.05)' }} onClick={onBack}>
                <ArrowLeft size={18} color={TEXT} />
              </button>
              <div className="flex items-center gap-1.5">
                <div className="w-6 h-1 rounded-full" style={{ background: GRADIENT }} />
                {[0,1,2,3].map(i => <div key={i} className="w-2 h-1 rounded-full" style={{ background: 'rgba(255,255,255,0.15)' }} />)}
              </div>
              <button className="w-10 h-10 rounded-full flex items-center justify-center"
          ...
          {mounted && (
            <div className="mt-6">
              <p className="text-[11px] tracking-[0.3em] font-black uppercase display mb-3 fade-up"
                style={{ color: 'transparent', background: GRADIENT, WebkitBackgroundClip: 'text', backgroundClip: 'text', animationDelay: '0.1s' }}>
                Step 1 of 5
              </p>
              <h1 className="display leading-[1] mb-2 fade-up"
                style={{ color: TEXT, fontWeight: 900, fontSize: '34px', animationDelay: '0.15s' }}>
                Enter your{' '}
                <span style={{ background: GRADIENT, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
                  Code.
                </span>
              </h1>
              <p className="text-[12px] leading-relaxed font-bold fade-up"
          ...
          {mounted && (
            <div className="mt-5 fade-up" style={{ animationDelay: '0.3s', minHeight: '120px' }}>
              {!verified ? (
                <>
                  {/* 지우기 버튼 — 건반 위 가운데 */}
                  <div className="flex justify-center mb-2" style={{ minHeight: '28px' }}>
                    {notes.length > 0 && (
                      <button onClick={removeNote}
                        ...
                        <span className="text-[11px] font-bold" style={{ color: TEXT_MUTED }}>지우기</span>
                      </button>
                    )}
                  </div>
                  ...
                  <div className="flex justify-between gap-1.5">
                    {[0,1,2,3,4].map(i => {
                      ...
                      return (
                        <div key={i} className="mono flex items-center justify-center transition-all"
                          style={{ flex: 1, height: '54px', fontSize: '20px', fontWeight: 800, color: isFilled ? TEXT : '#333', background: isFilled ? 'rgba(167,139,250,0.1)' : CARD, border: `2px solid ${isFilled ? '#a78bfa' : isCurrent ? '#444' : CARD_BORDER}`, borderRadius: '12px' }}>
                          {note || '·'}
                        </div>
                      );
                    })}
                ...
                <div className="text-center pop">
                  <p className="display"
                    style={{ fontSize: '46px', fontWeight: 900, background: GRADIENT, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', letterSpacing: '-0.04em', lineHeight: 1 }}>
                    {CORRECT_NAME}
                  </p>
                  <div className="flex items-center justify-center gap-1.5 mt-3">
                    <Check size={12} color="#6ee7b7" strokeWidth={3} />
                    <p className="text-[11px] font-black uppercase tracking-[0.25em] display" style={{ color: '#6ee7b7' }}>
                      Success
                    </p>
                  </div>
                </div>
          ...
          {mounted && (
            <div className="mt-auto mb-3 fade-up" style={{ animationDelay: '0.4s' }}>
                  ...
                  {whiteKeys.map((key) => {
                    ...
                    return (
                      <button key={key} onClick={() => addNote(key)} disabled={verified}
                        ...
                        style={{ background: isPressed ? 'linear-gradient(180deg,#c8b8ff 0%,#a78bfa 100%)' : 'linear-gradient(180deg,#fafafa 0%,#e0e0e0 100%)', boxShadow: isPressed ? 'inset 0 -2px 0 #8b6fd9,0 1px 2px rgba(0,0,0,.4)' : 'inset 0 -4px 0 #b8b8b8,0 2px 6px rgba(0,0,0,.4)', border: '1px solid #888', transform: isPressed ? 'translateY(2px)' : 'translateY(0)', transition: 'all 0.1s', cursor: verified ? 'default' : 'pointer' }}>
                        <span className="absolute bottom-2 left-1/2 -translate-x-1/2 mono text-[10px] font-black"
                          style={{ color: isPressed ? '#fff' : '#666' }}>{key}</span>
                      </button>
                    );
                  })}
                </div>
                  ...
                  {blackKeys.map(({ note, position }) => {
                    ...
                    return (
                      <button key={note} onClick={() => addNote(note)} disabled={verified}
                        ...
                        style={{ left: `calc(${(position/7)*100}% - 5%)`, width: '10%', height: '62%', top: 0, background: isPressed ? 'linear-gradient(180deg,#8b6fd9 0%,#6b4fb9 100%)' : 'linear-gradient(180deg,#2a2a2a 0%,#0a0a0a 100%)', boxShadow: isPressed ? 'inset 0 -1px 0 #5a3fa9,0 1px 2px rgba(0,0,0,.6)' : 'inset 0 -3px 0 #000,0 2px 5px rgba(0,0,0,.6)', border: '1px solid #000', transform: isPressed ? 'translateY(1px)' : 'translateY(0)', transition: 'all 0.1s', cursor: verified ? 'default' : 'pointer' }}>
                        <span className="absolute bottom-1.5 left-1/2 -translate-x-1/2 mono text-[8px] font-black" style={{ color: '#fff' }}>{note}</span>
                      </button>
                    );
                  })}
                </div>
              ...
              {!verified && notes.length === 0 && (
                <p className="mt-3 text-center text-[11px] font-bold" style={{ color: '#555' }}>
                  Hint : Fmaj7(#11)
                </p>
              )}
            </div>
          ...
          {mounted && (
            <button disabled={!verified} onClick={() => verified && onNext()}
              ...
              style={{ background: verified ? GRADIENT : '#1a1a1a', boxShadow: verified ? '0 8px 32px rgba(167,139,250,.3)' : 'none', animationDelay: '0.5s', opacity: verified ? 1 : 0.5, cursor: verified ? 'pointer' : 'not-allowed' }}>
              <span className="text-base font-black display" style={{ color: verified ? '#000' : '#444' }}>다음</span>
              <ArrowRight size={18} strokeWidth={3} style={{ color: verified ? '#000' : '#444' }} />
            </button>
          )}
